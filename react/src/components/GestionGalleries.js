  ...
  const onSubmit = () => {
    if (type == 'photo'){
      ...
      const submitOptions = {
        ...
        body: JSON.stringify({ name: name, video_url: video })
      };
      fetch('/api/videos/create/', submitOptions)
        .then(res => res.json())
        .then(
          (result) => {
            if (result.name == undefined)
        ...
        <div className='pic-modal'>
          <div ref={ref} className='add-modal-content'>
            ...
            <form ref={ref2} onSubmit={onSubmit}>
              ...
              {type == 'video' && (
                <TextField className="add-modal-textfield" color='secondary' value={video} type="text" label="Vidéo" onChange={e => setVideo(e.target.value)} fullWidth />
              )}
